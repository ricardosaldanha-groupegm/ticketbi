...
export default function SubticketAttachmentsList({ subticketId }: { subticketId: string }) {
  ...
  const fetchAttachments = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/subtickets/${subticketId}/attachments`)
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Erro ao carregar anexos')
      setAttachments(data)
    } catch (e) {
  ...
  if (loading) {
    return (
      <Card className="bg-slate-800 border-slate-700">
        <CardContent className="p-6 text-slate-200"><div className="text-center">A carregar anexos...</div></CardContent>
      </Card>
    )
  }
    ...
    <Card className="bg-slate-800 border-slate-700">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-slate-400">Ficheiros associados a esta tarefa</CardDescription>
          </div>
      ...
      <CardContent className="text-slate-200">
        {attachments.length === 0 ? (
          <div className="text-center py-8 text-slate-400">Nenhum anexo encontrado</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {attachments.map((attachment) => (
                <TableRow key={attachment.id}>
                  <TableCell className="font-medium"><div className="flex items-center gap-2"><File className="h-4 w-4" />{attachment.filename}</div></TableCell>
                  <TableCell><span className="text-sm text-slate-400">{attachment.mimetype}</span></TableCell>
                  <TableCell><span className="text-sm text-slate-400">{formatFileSize(attachment.size_bytes)}</span></TableCell>
                  <TableCell>{attachment.uploaded_by_user.name}</TableCell>
                  <TableCell><span className="text-sm text-slate-400">{formatDate(attachment.created_at)}</span></TableCell>
                  <TableCell>
                    <Button size="sm" onClick={() => handleDownload(attachment)}>
